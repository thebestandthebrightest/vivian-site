  ...
  return (
    <main className="flex min-h-svh items-center justify-center bg-background px-5 py-12 text-foreground sm:px-10 lg:px-14">
      <div className="flex w-full max-w-[46rem] flex-col items-center text-center">
        ...
        <nav
          className="mt-7 flex flex-col items-center gap-4 sm:mt-9 sm:gap-5 md:mt-10 md:gap-6"
        ...
        >
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="focus-ring group relative inline-flex font-display text-[clamp(2.7rem,6.6vw,5.15rem)] font-semibold uppercase leading-[0.88] text-foreground transition duration-300 ease-out hover:-translate-y-0.5 hover:opacity-75"
            >
